...
const perguntasQuiz = [
  {
    texto: "O que te empolga mais?",
    opcA: "Criar interfaces bonitas",
    opcB: "Modelar bancos de dados",
  },
  {
    texto: "Seu superpoder seria:",
    opcA: "CSS perfeito em qualquer tela",
    opcB: "API nunca cai",
  },
  {
    texto: "Você prefere resolver:",
    opcA: "Animações e UX",
    opcB: "Lógica e performance",
  },
];
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    fetch("https://api.adviceslip.com/advice")
      ...
      .catch(() =>
        setFrase("A persistência é o caminho do êxito. — Charles Chaplin"),
      );
  }, []);
    ...
    <div className="min-h-screen bg-slate-950 text-white">
        ...
        <div className="grid gap-12 lg:grid-cols-[1.1fr_0.9fr] items-center">
          <div className="space-y-6">
            ...
            <h1 className="text-4xl font-semibold tracking-tight sm:text-5xl">
              Olá, eu sou <span className="text-green-400">{nome}</span> —
              desenvolvedor fullstack focado em projetos web reais.
            </h1>
            <p className="text-lg leading-8 text-slate-300">{bio}</p>
            <p className="text-sm text-slate-400">
              Curso: <span className="text-white font-medium">{curso}</span>
            </p>
      ...
      <section className="mx-auto max-w-6xl px-6 pb-16">
        <div className="rounded-[2rem] border border-white/10 bg-white/5 p-6">
          <p className="text-sm uppercase tracking-[0.3em] text-green-300 mb-2">
            Calculadora do Curso
          </p>
          <h2 className="text-2xl font-semibold text-white mb-4">
            Seu progresso no tempo
          </h2>
          <div className="grid sm:grid-cols-3 gap-4">
            ...
            <div className="rounded-2xl bg-slate-950/80 p-4">
              ...
              <p className="mt-1 text-3xl font-bold text-green-400">
                {new Date().getFullYear() - anoInicio}
              </p>
            </div>
            <div className="rounded-2xl bg-slate-950/80 p-4">
              <p className="text-xs text-slate-500 uppercase tracking-widest">
              ...
              <p className="mt-1 text-3xl font-bold text-green-400">
                {mesesFormatura === 0 ? "Formado!" : mesesFormatura}
              </p>
            </div>
          </div>
        </div>
      </section>
      <section><PacManGame></PacManGame></section>
      ...
      <section id="skills" className="mx-auto max-w-6xl px-6 pb-16">
        ...
        <h2 className="text-3xl font-semibold text-white mb-6">
          Tecnologias e práticas que uso
        </h2>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {habilidades.map((skill) => (
            <div
              key={skill}
              className="rounded-3xl border border-white/10 bg-white/5 p-5 text-slate-200 hover:border-green-400/40 transition"
            >
              {skill}
            </div>
          ))}
      ...
      <section className="mx-auto max-w-6xl px-6 pb-16">
        <div className="rounded-[2rem] border border-white/10 bg-white/5 p-8">
          ...
          <h2 className="text-3xl font-semibold text-white mb-4">
            Você é mais Front-end ou Back-end?
          </h2>
          ...
          {quizAtivo && !resultado && (
            <div className="space-y-4">
              ...
              <div className="flex gap-4">
                <button
                  onClick={() => responderQuiz("front")}
                  className="flex-1 rounded-2xl border border-white/10 p-4 hover:border-green-400 hover:text-green-300 transition text-left"
                >
                  🎨 {perguntasQuiz[etapaQuiz].opcA}
                </button>
                <button
                  onClick={() => responderQuiz("back")}
                  className="flex-1 rounded-2xl border border-white/10 p-4 hover:border-green-400 hover:text-green-300 transition text-left"
                >
              ...
              <p className="text-xs text-slate-500">
                Pergunta {etapaQuiz + 1} de {perguntasQuiz.length}
              </p>
            </div>
          )}
          ...
          {resultado && (
            <div className="space-y-4">
              ...
              <button
                ...
                className="rounded-full border border-white/10 px-4 py-2 text-sm hover:border-green-300 transition"
              >
                Refazer quiz
              </button>
            </div>
      ...
      <section id="projects" className="mx-auto max-w-6xl px-6 pb-16">
        ...
        <h2 className="text-3xl font-semibold text-white mb-6">
          Trabalhos profissionais recentes
        </h2>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          {projetos.map((project) => (
            <article
              key={project.titulo}
              className="rounded-[2rem] border border-white/10 bg-white/5 p-6 transition hover:border-green-400/40 hover:bg-white/10"
            >
              ...
              <div className="mt-6 flex flex-wrap gap-2">
                {project.tags.map((tag) => (
                  <span
                    key={tag}
                    className="rounded-full border border-white/10 bg-green-500/10 px-3 py-1 text-[11px] uppercase tracking-[0.2em] text-green-300"
                  >
                    {tag}
                  </span>
                ))}
      ...
      <section className="mx-auto max-w-6xl px-6 pb-16">
        <div className="rounded-[2rem] border border-green-400/20 bg-green-500/5 p-8 text-center">
          <p className="text-sm uppercase tracking-[0.3em] text-green-300 mb-4">
            Frase do Dia (API pública)
          </p>
            ...
            {[
              {
                ...
                empresa: "Pemill Fundição e Usinagem",
                detalhe:
                  "Melhorei o Portal do Cliente com novas telas e automações, reduzindo retrabalho e acelerando processos.",
              },
              {
                titulo: "Freelancer Técnico",
                empresa: "Soluções para clientes B2B",
                detalhe:
                  "Entreguei sistemas administrativos e sites com foco em desempenho, usabilidade e manutenção.",
              },
            ].map((item) => (
              <div
                key={item.titulo}
          ...
          <div className="grid gap-8 lg:grid-cols-2 items-start">
            <div className="space-y-4 text-slate-300">
              <p className="leading-7">
                Estou disponível para novos desafios em desenvolvimento web,
                automação e integrações corporativas.
              </p>
              <div>
                <p className="text-sm uppercase tracking-[0.2em] text-slate-500">
                  Email
                ...
                <>
                  ...
                  {formErros.length > 0 && (
                    <ul className="space-y-1">
                      {formErros.map((e) => (
                        <li key={e} className="text-sm text-red-400">
                          ⚠ {e}
                        </li>
                      ))}
                    </ul>
